...
DINO_PCA_FEATURE_PATTERN = re.compile(
    r"^dino_pca_(global|mouth|eyes|brow|nose)_\d+$"
)


def is_dino_pca_feature(
    name: str,
) -> bool:
    """
    True only for actual PCA coordinates such as:
        dino_pca_global_00
        dino_pca_mouth_07

    Explicitly excludes QC/helper columns such as:
        dino_pca_available
    """
    return bool(
        DINO_PCA_FEATURE_PATTERN.fullmatch(
            str(name)
        )
    )
...
def main():
    ...
    print(
        "V4 STRATIFIED LOSO EVALUATION v3 (QC-FEATURE-SAFE)"
    )
    print(
        "============================================================"
    )
    ...
    for held_out in args.participants:
        ...
        print(
            f"  DINO model features: {len(dino_cols)} "
            f"(QC flag dino_pca_available kept separate)"
        )

        if dino_cols != expected_sorted:
            missing = sorted(
                set(
                    expected_sorted
                )
            ...
            raise RuntimeError(
                f"{held_out}: DINO PCA feature schema mismatch. "
        ...
        if len(
            ...
            raise RuntimeError(
                f"{held_out}: unexpected DINO PCA feature count "
                f"{len(dino_cols)}."
            )

        if "dino_pca_available" not in fold_subject_frames[
            held_out
        ].columns:
            raise RuntimeError(
                f"{held_out}: missing dino_pca_available QC flag."
            )
        ...
        for modality_name, columns in modality_sets.items():
            leaked = sorted(
                forbidden_model_features
                & set(
                    columns
            ...
            if leaked:
                raise RuntimeError(
                    f"{held_out}/{modality_name}: QC/label columns leaked "
                    f"into model features: {leaked}"
                )
    ...
    config = {
        ...
        "dino_feature_policy": (
            "only dino_pca_<region>_<component> coordinates are model inputs; "
            "dino_pca_available is QC-only and never a learned feature"
        ),
        "action_challenge": (
