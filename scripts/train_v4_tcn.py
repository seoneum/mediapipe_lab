...
def main():
    ...
    for fold_idx, held_out in enumerate(
        ...
        encoder_path = out_dir / f"encoder_held_out_{held_out}.pt"
        encoder_digest = export_temporal_encoder_checkpoint(
            encoder_path,
            ...
            normalization_mean=[float(stats.center[name]) for name in feature_cols],
            normalization_std=[float(stats.scale[name]) for name in feature_cols],
            metadata={
                "held_out_participant": held_out,
                "train_participants": train_subjects,
                "best_epoch": int(best_epoch),
                "normalization": "outer-train calibration robust center/scale",
            },
        )
        encoder_checkpoints[held_out] = {
            "path": str(encoder_path.relative_to(ROOT)),
            "sha256": encoder_digest,
        }
