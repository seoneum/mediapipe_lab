    ...
    def review(
        ...
        *,
        ...
        prompt = (
            ...
            "메타데이터에 movement_summary, similarity_to_previous, selection_explanation이 있으면 수치를 바꾸지 말고, "
            "어느 부위가 얼마나 변했고 앞선 개인 발생들과 무엇이 비슷해 이벤트로 선정됐는지 일반인이 이해할 말로 설명하세요. "
            "임베딩 유사도는 확률이나 일치율이 아닙니다. 반드시 ‘시간 흐름 특징 유사도’라고 부르고, "
            "‘같은 행동으로 확인됐다’고 쓰지 마세요. 영상에서 보이지 않는 이전 발생의 구체 동작도 지어내지 마세요. "
            "감정, 의도, 집중도, 순응도, 선호, 진단을 추론하거나 확정하지 마세요. "
            "아동을 평가하거나 점수화하지 말고, 불확실하면 불확실하다고 쓰세요. "
            "출력은 1) 관찰 가능한 변화 2) 이전 발생과의 유사성 및 선정 이유 3) 확인 불가/한계 4) 사람이 다시 볼 지점의 네 구역으로 간결하게 작성하세요.\n\n"
