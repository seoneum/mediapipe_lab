    ...
    def review(
        ...
        *,
        ...
        system = (
            ...
            "구조화 메타데이터의 움직임 부위, 변화량, 이전 발생 유사도와 선정 기준은 수치를 바꾸지 말고 쉬운 말로 풀어 쓰세요. "
            "임베딩 유사도는 확률이나 일치율이 아닙니다. 반드시 ‘시간 흐름 특징 유사도’라고 부르고, "
            "‘같은 행동으로 확인됐다’고 쓰지 마세요. 이전 영상에 없던 구체 동작도 지어내지 마세요. "
            "감정, 의도, 집중도, 순응도, 선호 또는 진단을 추론하거나 점수화하지 마세요. "
            "불확실하면 확인 불가라고 쓰고 공식 기록이나 의사결정을 자동 생성하지 마세요."
        )
        prompt = (
            "시간 순서대로 추출한 이벤트 프레임입니다. "
            "1) 관찰 가능한 변화 2) 이전 발생과의 유사성 및 선정 이유 3) 확인 불가/한계 4) 사람이 다시 볼 지점 순서로 간결하게 작성하세요.\n\n"
