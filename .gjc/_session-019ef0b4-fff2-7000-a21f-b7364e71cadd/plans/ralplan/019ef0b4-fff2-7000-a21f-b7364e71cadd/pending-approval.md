...
| Supervisor(운영자/보호자) | 있음. 세션 시작 승인, 대상 선택 승인, 추종 재개 승인 | 있음. soft stop 요청 가능 | 있음. 단 E-stop/contactor fault 해제 후에만 | 취약 사용자 세션 필수 |
| Software watchdog | 없음 | 있음. heartbeat 상실, 지연 초과 시 즉시 정지 | 없음 | 자동 복귀 금지 |
| Hardware E-stop | 없음 | 절대적 있음. 인간 접근 상황의 즉시 정지 채널 | 없음 | 물리 리셋 후에도 supervisor 재승인 필요 |
...
- **인간 즉시 정지(human-immediate-stop)**: 사람 손이 닿는 위치의 하드웨어 E-stop 또는 동등한 bounded stop path만 인정한다. 이 채널은 지연 허용이 아니라 **추가 접근 거리 상한**으로 관리한다.
- **소프트 supervisor stop**: 운영자/보호자가 UI, 리모컨, 소프트 명령으로 요청하는 정지다. 이는 E-stop 대체가 아니라 보조 채널이며, safe state 도달 시간과 추가 접근 거리 예산으로 관리한다.
- 취약 사용자 근접 상황에서 즉시 멈춤이 필요한 경우 soft stop이 아니라 human-immediate-stop을 사용해야 한다.
...
### Phase 2.5: Final-platform requalification gate
- 대상: 실제 **자가균형 휠레그드 최종 동역학 플랫폼**
- 원칙: safety mule에서 통과한 증거를 그대로 전이하지 않는다. 사람 대상 시험 전에 최종 플랫폼에서 **재적격화**한다.
- 사람 대상 시험 진입 전 재검증 필수 항목:
  1. E-stop 즉시 정지 경로
  2. brownout 경고/차단 동작
  3. watchdog timeout 시 safe state 진입
  4. D435 fault 시 추종 금지 및 정지
  5. 라이다 fault 시 자율 추종 금지 및 제한 모드 전환
  6. 정지거리 계약
  7. anti-fall / anti-roll 동작
  8. restart inhibit 조건 유지
- 통과 조건: 위 8개 항목을 최종 플랫폼 동역학에서 **각 100% 재현 검증**하고, 사람 없는 시험에서 release blocker 0건
- 실패 조건: mule에서는 통과했더라도 최종 플랫폼에서 항목 1건이라도 미충족이면 인간 대상 Person following 금지
...
- Phase 2.5 재적격화 통과 전까지 인간 대상 시험 금지
...
6. **최종 자가균형 휠레그드 플랫폼 Phase 2.5 재적격화**
7. 성인 감독자 대상 사람추종 검증
8. 노인 우선 보조 시나리오 파일럿
9. 영아 관련은 보호자 지원 범위만 별도 심사
...
- **human-immediate-stop 추가 접근 거리 상한: 정지 요청 후 추가 접근 0.15 m 이하**
- **soft supervisor stop safe-state 도달 목표: 정지 요청 후 1.0초 이내 또는 추가 접근 0.25 m 이하 중 더 보수적인 조건 충족**
...
#### Phase 1 pass/fail
- 통과:
  - 속도 0.2 m/s 이하 bench 제어에서 반복 오동작 없이 전류 제한 동작
  - brownout 모의 주입 시 추종 명령 무시와 restart inhibit 100% 재현
  - E-stop 후 자동 재시작 0회, supervisor 재승인 없이는 구동 재개 불가
  - human-immediate-stop 경로에서 추가 접근 거리 상한 이내 safe state 도달
- 실패:
  - 1회라도 uncontrolled coast 또는 restart inhibit 누락 발생

#### Phase 2 pass/fail
- 통과:
  - 사람 없는 실내에서 0.4 m/s 이하 주행 중 정지거리 목표 충족
  - D435 fault와 라이다 fault 각각에서 degraded mode 규칙 100% 일치
  - 수동 밀기 조건과 anti-roll 확인 완료
  - soft supervisor stop에서 safe state 도달 목표 충족
- 실패:
  - 라이다 fault 시 계속 자율 주행하거나, 정지 후 재시작 권한이 모호함
  - soft stop 또는 E-stop이 거리 예산을 초과함

#### Phase 2.5 pass/fail
- 통과:
  - 최종 자가균형 휠레그드 플랫폼에서 E-stop, brownout, watchdog, D435 fault, 라이다 fault, 정지거리, anti-fall/anti-roll, restart inhibit를 100% 재검증
  - 사람 없는 시험에서 release blocker 0건
  - human-immediate-stop 및 soft supervisor stop이 최종 플랫폼 동역학에서도 거리 예산을 재충족
- 실패:
  - mule 증거를 그대로 사용하거나, 위 항목 중 하나라도 최종 플랫폼에서 미재검증

#### Phase 3 pass/fail
- 통과:
  - 성인 감독자 대상 제한 공간에서 10회 연속 추종 중 충돌 0건
  - confidence <0.85가 300ms 지속될 때 100% 정지
  - human-immediate-stop 요청 시 추가 접근 0.15 m 이하에서 safe state 도달
  - soft supervisor stop 요청 시 1.0초 이내 또는 추가 접근 0.25 m 이하 중 더 보수적인 조건으로 safe state 도달
  - 오추종 0건
- 실패:
  - 이격 0.6 m 미만 진입 1회라도 발생
  - supervisor 승인 없는 대상 전환 발생

#### Phase 4 pass/fail
- 통과:
  - 노인 우선 파일럿 세션에서 misuse 종료 규칙 100% 준수
  - supervisor 1:1 배치 준수
  - 비진단 요약만 생성하고 의료적 추정 0건
- 실패:
  - grabbing misuse를 허용하거나, 영아 직접 추종으로 범위가 확장됨

### 시나리오별 concrete verification
#### 시나리오 A: 가림 후 재식별 실패
- Setup: 복도, 대상 1명 + 비대상 1명
- Fault injection: 대상이 코너를 돌며 500ms 이상 부분 가림
- Expected stop behavior: confidence 0.85 미만이 300ms 지속되면 즉시 safe state 진입, supervisor 재승인 전 재출발 금지
- Required logs: confidence 곡선, stop trigger 시점, restart attempt, supervisor 승인 로그
- Release blocker: 오추종 1회라도 발생 시 즉시 차단

#### 시나리오 B: brownout
- Setup: 벤치 또는 사람 없는 베이스, 저전압 모의
- Fault injection: 저전압 경고 구간과 차단 구간 순차 진입
- Expected stop behavior: 경고 구간 2초 이내 속도 상한 0.3 m/s 이하, 차단 구간 진입 시 능동 감속 후 정지, 자동 추종 재개 금지
- Required logs: 배터리 전압, 속도 상한 상태, stop mode, restart inhibit 상태
- Release blocker: 차단 구간에서 추종 지속 1회라도 발생 시 차단

#### 시나리오 C: 라이다 fault
- Setup: 사람 없는 베이스
- Fault injection: 라이다 데이터 중단
- Expected stop behavior: 자율 추종 금지, 수동 저속 제한 모드 또는 정지 유지
- Required logs: fault detect 시점, mode transition, move command inhibit
- Release blocker: fault 중 자율 이동 유지 시 차단

#### 시나리오 D: human-immediate-stop
- Setup: 사람 근접 모의 상황, supervisor 대기
- Fault injection: 예고 없는 E-stop 입력
- Expected stop behavior: 추가 접근 0.15 m 이하에서 safe state 도달
- Required logs: E-stop 입력, contactor 상태, 토크 상태, 정지거리, 재시작 시도 로그
- Release blocker: E-stop 후 자동 복귀 1회라도 발생하거나 추가 접근 거리 상한 초과 시 즉시 차단

#### 시나리오 E: grabbing misuse
- Setup: 파일럿 모의 세션, supervisor 동반
- Fault injection: 사용자가 로봇을 지지대처럼 잡는 행동 시도
- Expected stop behavior: 즉시 세션 종료, 이후 supervisor 리셋 전 재개 금지
- Required logs: misuse flag, stop reason, session end marker
- Release blocker: misuse 상황에서 세션 지속 시 차단

#### 시나리오 F: 최종 플랫폼 재적격화
- Setup: 자가균형 휠레그드 최종 플랫폼, 사람 없는 시험 환경
- Fault injection: E-stop, brownout, watchdog timeout, D435 fault, 라이다 fault를 각각 독립 주입
- Expected stop behavior: mule와 동일한 안전 계약을 최종 동역학에서도 재충족, anti-fall/anti-roll 유지, restart inhibit 유지
- Required logs: 항목별 stop latency, additional approach distance, anti-fall 상태, restart inhibit 상태
- Release blocker: 1개 항목이라도 최종 플랫폼에서 미충족이면 사람 대상 시험 금지

## 11. ADR
### Decision
취약 사용자 대상 초기 사람추종 보조 로봇은 **노인 우선**, **옵션 B safety mule**, **D435 + 2D 안전 라이다 + moteus r4.11 + Linux 런타임** 조합으로 단계 검증한다.

### Drivers
- 안전 우선과 즉시 정지 가능성
- 공급 가능성과 문서 성숙도
- 사람추종 가치 검증과 하부 안정화의 단계 분리 가능성
...
- 휠레그드 정체성은 Phase 2 go/no-go 이후로 판단하되, 최종 플랫폼에서는 Phase 2.5 재적격화를 거친다.

### Consequences
- 초기 BOM 증가
- 자가균형 휠레그드 완성 시점은 늦어질 수 있음
...
- 외부 사실 반영 근거:
  ...
  - stage-02 revision과 architect/critic 피드백을 반영해 Phase 2.5 재적격화 게이트와 거리 예산 기반 stop 계약을 추가함
- 이번 단계는 계획 전용이므로 테스트, 포매터, 소스 수정은 수행하지 않음

## Risks and mitigations
- 위험: safety mule이 최종 제품으로 오인될 수 있음
  - 완화: 옵션 B를 명시적으로 mule로 표기하고 Phase 2.5 재적격화 게이트 추가
- 위험: stop 채널이 운용 편의 중심으로 해석될 수 있음
  - 완화: human-immediate-stop과 soft supervisor stop을 분리하고 거리 예산 기반 acceptance로 고정
