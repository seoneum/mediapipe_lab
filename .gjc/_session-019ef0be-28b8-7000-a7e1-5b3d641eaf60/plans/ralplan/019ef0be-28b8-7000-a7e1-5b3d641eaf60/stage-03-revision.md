...
- Expected stop behavior: 즉시 bounded stop path 진입, 추가 접근 0.15 m 이하에서 safe state 도달, 이후 supervisor와 fault clear 전까지 재시작 금지
...
- Required logs: 접촉 스위치/범퍼 이벤트, supervisor stop 또는 E-stop 이벤트, 세션 종료 코드
...
- Required logs: fault 종류, 자세 추정, 속도, 정지거리, safe state 도달 시간, restart inhibit 상태
- Release blocker: 항목 하나라도 재검증 실패 시 인간 대상 Phase 3 차단
